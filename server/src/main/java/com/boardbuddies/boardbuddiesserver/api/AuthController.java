...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<SocialLoginResponse>> completeSignup(
        ...
        try {
            ...
            // 회원가입 완료 처리
            SocialLoginResponse response = authService.completeSignup(userId, request);
     ...
     * 임시 토큰에서 userId 추출
     */
    private Long extractUserIdFromTempToken(String tempAccessToken) {
        try {
            // JWT 검증
            if (!jwtUtil.validateToken(tempAccessToken)) {
                throw new RuntimeException("유효하지 않은 토큰입니다.");
            }
            
            // 토큰 타입 확인 (temp 토큰인지)
            String tokenType = jwtUtil.getTokenType(tempAccessToken);
            if (!"temp".equals(tokenType)) {
                throw new RuntimeException("임시 토큰이 아닙니다.");
            }
            
            // userId 추출
            return jwtUtil.getUserIdFromToken(tempAccessToken);
        } catch (Exception e) {
            throw new RuntimeException("유효하지 않은 토큰입니다.");
        }
    }
